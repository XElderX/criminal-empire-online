...
final class AuthService
{
    public function register(array $data): array
    {
        ...
        $stmt = Database::pdo()->prepare('INSERT INTO users (username,email,password,role,cash,energy,max_energy,created_at,updated_at) VALUES (?,?,?,?,5000,100,100,NOW(),NOW())');
        $stmt->execute([
            trim($data['username']),
    ...
    private function issueToken(int $userId): array
    {
        ...
        $user = Database::pdo()->query('SELECT id,username,email,role,cash,bank_cash,energy,max_energy,heat,level,experience,strength,intelligence,charisma,combat,leadership FROM users WHERE id=' . $userId)->fetch();
